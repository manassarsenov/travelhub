...
// Escapes parent overflow:hidden by positioning in viewport coords
function _positionFixedDropdown(anchorEl, dropEl, opts) {
    ...
    dropEl.style.top   = (rect.bottom + gap) + 'px';
...
document.addEventListener('DOMContentLoaded', () => {
    window.addEventListener('scroll', () => {
        ...
        if (suggBox?.classList.contains('active')) {
            ...
            _positionFixedDropdown(input?.closest('.search-input-group'), suggBox, { minW: 480 });
        }
    }, { passive: true });
    ...
    document.addEventListener('click', (e) => {
        ...
        const suggBox = document.getElementById('search-suggestions');
        if (group && !group.contains(e.target) && suggBox && !suggBox.contains(e.target)) {
            hideSuggestions();
        }
    });
});
...
function _showSuggestionsLoading() {
    ...
    if (!box.classList.contains('active')) {
        _positionFixedDropdown(input?.closest('.search-input-group'), box, { minW: 480 });
...
// type icons used in suggestions
const _typeIcon = {
...
function _renderHeroSuggestions(results, q) {
    ...
    const labels   = { region: 'Regions', country: 'Countries', city: 'Cities', destination: 'Destinations' };
    const grouped  = {};
    results.forEach(r => {
    ...
    order.forEach(type => {
        ...
        html += '<div class="hs-section-label"><i class="' + (_typeIcon[type] || 'fas fa-search') + '"></i> ' + labels[type] + '</div>';
        grouped[type].slice(0, 5).forEach(r => {
            const idx   = results.indexOf(r);
            const thumb = r.image
                ? '<div class="hs-thumb"><img src="' + r.image + '" alt="" loading="lazy" ' +
                  'onerror="this.parentNode.innerHTML=\'<i class=\\\"' + (_typeIcon[type] || 'fas fa-search') + '\\\"></i>\'"></div>'
                : '<div class="hs-thumb hs-thumb-icon"><i class="' + (_typeIcon[type] || 'fas fa-search') + '"></i></div>';
            html +=
                '<div class="suggestion-item hs-item" onclick="_selectHeroResult(' + idx + ')" ' +
                'tabindex="0" onkeydown="if(event.key===\'Enter\') _selectHeroResult(' + idx + ')">' +
                thumb +
                '<div class="hs-info">' +
                '<div class="hs-title">' + _hl(r.title, q) + '</div>' +
                '<div class="hs-sub">' + (r.subtitle || '') + '</div>' +
                '</div>' +
                (r.count != null ? '<div class="hs-meta"><div class="hs-count">' + r.count + ' destinations</div></div>' : '') +
                '</div>';
        });
    });

    html +=
        '<div class="hs-footer" onclick="searchDestinations()">' +
        '<i class="fas fa-search"></i> Show all results for "<strong>' + q + '</strong>"' +
...
function _selectHeroResult(idx) {
    ...
    // City → filterByCity; everything else → filterByQuery
